...
### Reproducibility: now regenerated inside the repository

The measurement file is committed at `data/gl765_legacy.csv` and the fit is regenerated by
...
which writes `results/frozen/legacy_target_fit.csv`. Supporting code is in `src/raa_orbit_model/legacy_target.py`, with 21 tests in `tests/test_legacy_target.py` that pin the result against the manuscript table.

The regenerated fit reproduces the values reported above, including the formal uncertainties:

| quantity | regenerated | as reported above |
|---|---|---|
| chi2 | 104.504 | 104.629 |
| dof | 100 | 100 |
| reduced chi2 | 1.0450 | 1.046 |
| P (yr) | 11.7285 +/- 0.0732 | 11.7284 +/- 0.0732 |
| e | 0.24887 +/- 0.01005 | 0.24888 +/- 0.01005 |
| i (deg) | 81.836 +/- 1.371 | 81.834 +/- 1.371 |
| Omega (deg) | 289.045 +/- 3.269 | 289.07 +/- 3.27 |
| omega_rel (deg) | 251.826 +/- 2.246 | 251.79 +/- 2.25 |
| total mass (Msun) | 1.5898 +/- 0.0527 | 1.5897 |
| orbital parallax (mas) | 35.443 +/- 2.244 | 35.44 +/- 2.24 |
| chi2 at fixed 54.27 mas | 168.823 (1.672) | 168.94 (1.673) |
| chi2 at fixed 31.0 mas | 108.580 (1.075) | 108.70 (1.076) |

Elements and uncertainties agree to four or five significant figures. The three chi-square values are each lower by about 0.12, a constant offset rather than a convergence difference, so one measurement is presumably weighted marginally differently in the original run. It does not affect any reported element.

### Two data hazards this file contains

Both would corrupt the fit silently, and both are now guarded and tested.

1. **Two time systems.** Radial velocities are Julian dates minus 2 400 000; visual measurements are decimal years. Left unconverted, the velocities would sit millennia away from an orbit of twelve years. The reader converts them and refuses a file whose blocks do not overlap in time.
2. **Case-significant header keys.** `W` is the node and `w` the argument of periastron. A case-insensitive lookup conflates them and yields a wrong orbit; the reader now matches case first.

### Component labels

`K1 > K2` in this file, so the `Va` curve belongs to the *less massive* component and the fitted `M1 < M2`. The labels therefore do not map onto the A/B convention of Balega et al. (2007), and only the label-invariant total mass should be compared. That total, 1.5898 Msun, is 0.26 per cent from the published 1.594 Msun.

### Still outstanding

The header coordinates (RA 19.404, Dec 76.1812) disagree with the SIMBAD position and must never be used for Gaia identification; `dr3_target.py` uses the HIP2 crossmatch instead.
